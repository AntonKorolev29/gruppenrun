...
async def gruppenrun_uktus_main(message: types.Message):
    ...
    main_text = (
        ...
        "<b>3 группы сложности:</b>\n"
        "🟢 Начинающая (7-9 км)\n"
        "🟡 Средняя (9-16 км)\n"
        "🔴 Продвинутая (14-20+ км) — для подготовленных\n\n"
        "<b>База:</b>\n"
...
async def uktus_register_callback(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    if user and user.get("name") and user.get("phone"):
        ...
        await callback_query.message.answer(
            "🏔 Группенран Уктус - 3 группы сложности:\n\n"
            "🟢 Начинающая (7-9 км)\n"
            "🟡 Средняя (9-14 км)\n"
            "🔴 Продвинутая (14-20+ км)\n\n"
            "Разделение по группам происходит перед старта на брифинге\n\n"
